...
__isr_vector:
    .long __stack
    .long Reset_Handler                                     /* Reset_Handler            PURE ASSEMBLY       */
    .long NMI_Handler                                       /* NMI                      EMPTY               */
    .long HardFault_Handler                                 /* HardFault                EMPTY               */
    .long MemManage_Handler                                 /* MemManage                EMPTY               */
    .long BusFault_Handler                                  /* BusFault                 EMPTY               */
    .long UsageFault_Handler                                /* UsageFault               EMPTY               */
    .long 0                                                 /* Reserved                 WTF                 */
    .long 0                                                 /* Reserved                 WTF                 */
    .long 0                                                 /* Reserved                 WTF                 */
    .long 0                                                 /* Reserved                 WTF                 */
    .long vPortSVCHandler                                   /* SVCHandler               MIT FreeRTOS        */
    .long DebugMon_Handler                                  /* DebugMonitor             EMPTY               */
    .long 0                                                 /* Reserved                 WTF                 */
    .long xPortPendSVHandler                                /* PendSVHandler            MIT FreeRTOS        */
    .long xPortSysTickHandler                               /* SysTickHandler           MIT FreeRTOS        */
                                                            /* External Interrupts                          */
    ...
    .size __isr_vector, . - __isr_vector                    /*  Size for Linker                 */
/************************************************************************************************/
/* Flash Configuration                                                                          */
...
/************************************************************************************************/
    ...
    .globl   Reset_Handler
    .weak    Reset_Handler
    .type    Reset_Handler, %function  /* Regsiter Reset_handler                                */
Reset_Handler:                  /* Turn OFF Watchdog, set Stack, clear BSS and call main()      */
    cpsid   i                   /* CPS I-interrupts D-disable                                   */
    .equ    VTOR, 0xE000ED08    /* Assign 32-bit 0xE000ED08 value to VTOR symbol                */
    ldr     r0, =VTOR           /* load the value 0xE000ED08 into R0                            */
    ldr     r1, =__isr_vector   /* load the address of label __isr_vector into R1               */
    str     r1, [r0]            /* Store VTOR --> 0xE000ED08 into __isr_vector                  */
    ldr     r2, [r1]            /* Load __StackTop into R2                                      */
    msr     msp, r2             /* Load R2 into Master Stack Pointer                            */
    ldr     r0,=SystemInit      /* Load the address of function SystemInit into R0              */
    blx     r0                  /* Copy address of next instruction to link register            */
                                /* Jump to SystemInit function and change the instruction set   */
                                /* Thumb --> ARM                                                */
                                /* Arm --> Thumb                                                */
    ldr    r1, =__etext         /* R1 = Address --> Code End                                    */
    ldr    r2, =_sdata          /* R2 = Address --> Data Start                                  */
    ldr    r3, =_edata          /* R3 = Address --> Data End                                    */
.LC0:                           /* Link Data RAM --> ROM                                        */
    cmp     r2, r3              /* R2 - R3 = __data_start__ - __data_end__                      */
    ittt    lt                  /* If-then-then-then --> Less than for next 3 instructions      */
    ldrlt   r0, [r1], #4        /* Data from memory address [R1] + 4 --> R0 register            */
    strlt   r0, [r2], #4        /* Data from R0 register --> memory address [R2] + 4            */
    blt    .LC0                 /* Jump to LC0 if subtraction is Less than ZERO                 */
                                /* Move forward subtraction is more than zero                   */
    ldr r1, =__bss_start__      /* R1 = Address --> BSS Start                                   */
    ldr r2, =__bss_end__        /* R2 = Address --> BSS End                                     */
    movs    r0, 0               /* Clear --> R0                                                 */
.LC2:                           /* Clear BSS                                                    */
    cmp     r1, r2              /* R1 - R2 = __bss_start__ - __bss_end__                        */
    itt    lt                   /* If-then-then --> Less than for 2 instructions                */
    strlt   r0, [r1], #4        /* Zero Data --> memory address [R1] --> offset 4               */
    blt    .LC2                 /* Jump to LC2 if still Less than Zero                          */
    cpsie   i                   /* CPS I-interrupts E-enable                                    */
    ldr   r0,=main              /* Load main -----> Address                                     */   
    blx   r0                    /* Call main -----> MIT FreeRTOS !-)                            */
    .pool                       /* Same what .ltorg Directive --> literal pool                  */
    .size Reset_Handler, . - Reset_Handler /* Size of the function --> for linker               */
/************************************************************************************************/
    .align  1
    .thumb_func
    .weak DefaultISR
